...
export class DataAnalyzer {
  ...
  // Generate a simple text-based histogram for a column
  generateHistogram(data: DataTable, columnName: string, buckets: number = 10): string {
    const columnIndex = data.headers.indexOf(columnName);
    if (columnIndex === -1) {
      throw new Error(`Column "${columnName}" not found`);
    }
    ...
    // Generate the histogram text
    const maxCount = Math.max(...bucketCounts);
    const histogramWidth = 40; // characters for the bar
    const lines: string[] = [];
    
    lines.push(`Histogram for ${columnName}:`);
    lines.push('-'.repeat(60));
    
    for (let i = 0; i < buckets; i++) {
      const bucketStart = min + i * bucketSize;
      const bucketEnd = i === buckets - 1 ? max : min + (i + 1) * bucketSize;
      const barLength = Math.round((bucketCounts[i] / maxCount) * histogramWidth);
      const bar = '█'.repeat(barLength);
      
      lines.push(
        `${bucketStart.toFixed(2)} - ${bucketEnd.toFixed(2)}: ` +
        `${bar} (${bucketCounts[i]})`
      );
    }
